...
int echo_server(int sockfd,struct clients_information* INFO_CLIENTS) {
   ...
   if (send(sockfd, online_users, msgstruct.pld_len, 0) <= 0) {
            ...
            if(COPY->fd == sockfd ){
                if(strcmp(COPY->salon,"")==0){
                    ...
                    // the pseudo has been accepted
                        ...
                        sprintf(chaine,"You have created channel %s \n",msgstruct.infos );
                    ...
                    // the pseudo has been accepted
                        ...
                        sprintf(chaine,"You have changed the  channel %s \n",msgstruct.infos );
                        strcpy(COPY->salon,msgstruct.infos);
                    }
                    a=0;
    ...
    // if the user wants to get the list of the existing channels
        ...
        memset( salon,'\0',MSG_LEN);
            if(COPY->status ==1 && strcmp(COPY->salon,"")!=0){


               sprintf(salon,"\t - %s",COPY->salon);
   ...
   if (send(sockfd, channels, msgstruct.pld_len, 0) <= 0) {
    ...
    // if the user want to join an existing channel
    ...
    strncpy(msgstruct.nick_sender, COPY->pseudo,strlen(COPY->pseudo));
    msgstruct.type = ECHO_SEND;
    strncpy(msgstruct.infos,disconnected_ch,strlen(disconnected_ch));
  // Sending structure
              if (write(sockfd, &msgstruct, sizeof(msgstruct)) <= 0) {
       perror("Error while reading111");
    }
        ...
        //first case
           if(i == 0 && (fds[i].revents & POLLIN)){
            ...
            fds[0].revents = 0;
               ...
               strcpy(new_client->pseudo,"grouupp");
